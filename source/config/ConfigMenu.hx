...
class ConfigMenu extends FlxUIStateExt
{
	...
	function setupOptions(){
		...
		var fpsCap = new ConfigOption("FRAMERATE", #if (desktop || mobile) ": " + (allowedFramerates[framerateValue] == 999 ? "uncapped" : ""+allowedFramerates[framerateValue]) #else ": disabled" #end, #if desktop "Uncaps the framerate during gameplay.\n(Some menus will limit framerate but gameplay will always be at the specified framerate.)" #else "Disabled on Web builds." #end);
